"""The core navigation tools: how the agent reads a document.

Four tools mirror how a person reads a long PDF: see the structure (``get_outline``),
search for evidence (``search``), read a section closely (``read_section``), or read a raw
page (``read_page``). Each is self-describing - ``spec`` tells the LLM how to call it,
``run`` executes it - so the agent loop never changes (DESIGN.md §3.4, principle #1).
"""
...
class SearchTool(Tool):
    ...
    def run(self, **kwargs: object) -> ToolResult:
        query = first(kwargs, "query", "q")
        if not isinstance(query, str) or not query.strip():
            return ToolResult(content="search needs a non-empty 'query' string.", ok=False)
        top_k = as_int(first(kwargs, "top_k", "k"), default=self._default_top_k)
...
class ReadSectionTool(Tool):
    ...
    def run(self, **kwargs: object) -> ToolResult:
        section_id = first(kwargs, "section_id", "id", "section")
